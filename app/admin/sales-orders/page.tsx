...
type SaleOrderStatus = 'RECEIVED' | 'PROCESSING' | 'COMPLETED' | 'CANCELED';
...
type SaleOrder = {
  ...
  saleOrderProducts: SaleOrderItem[];
...
const initialForm: SaleOrderForm = {
  partnerId: '',
  partnerKeyword: '',
  items: [
    {
...
const initialErrors: SaleOrderFormErrors = {};
...
export default function SalesOrdersPage() {
  ...
  const [form, setForm] = useState<SaleOrderForm>(initialForm);
  const [formErrors, setFormErrors] =
    useState<SaleOrderFormErrors>(initialErrors);
  ...
  const getStatusLabel = (status: SaleOrderStatus) => {
    switch (status) {
      case 'RECEIVED':
        return '접수';
      case 'PROCESSING':
  ...
  const getStatusClassName = (status: SaleOrderStatus) => {
    switch (status) {
      case 'RECEIVED':
  ...
  const getOrderTotalPrice = (order: SaleOrder) => {
    return order.saleOrderProducts.reduce((acc, item) => {
      return acc + item.quantity * item.unitPrice;
    }, 0);
  };

  const getItemSummaryText = (order: SaleOrder) => {
    if (order.saleOrderProducts.length === 0) {
      return '-';
    }

    const firstItem = order.saleOrderProducts[0];
    const extraCount = order.saleOrderProducts.length - 1;
  ...
  const fetchSaleOrders = async (keyword = '', page = 1) => {
    try {
      setLoading(true);
      setListError('');
      ...
      const response = await fetch(
        `/api/sale-orders?${searchParams.toString()}`,
        {
          method: 'GET',
          cache: 'no-store',
        },
      ...
      const data: SaleOrderListResponse = await response.json();
      ...
      setSaleOrders(Array.isArray(data.items) ? data.items : []);
  ...
  const fetchCustomers = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.message || '고객사 목록을 불러오는 중 오류가 발생했습니다.',
        );
      }

      const items = Array.isArray(data.items) ? data.items : [];
      const customerItems = items.filter(
        (partner: Partner) => partner.type === 'CUSTOMER' && partner.isActive,
      );
  ...
  const fetchProducts = async () => {
    try {
      ...
      const saleProducts = items.filter(
        (product: Product) => product.isSale && product.currentStock > 0,
      );
  ...
  const handleSearch = async () => {
    ...
    setCurrentPage(1);
    await fetchSaleOrders(inputValue, 1);
  };
  ...
  const handlePageChange = async (page: number) => {
    ...
    await fetchSaleOrders(search, page);
  };

  const getFilteredCustomers = (keyword: string) => {
  ...
  const handleSelectPartner = (partnerId: string) => {
    const selectedPartner = customers.find(
  ...
  const getSelectedProductStock = (productId: string) => {
    const selectedProduct = products.find(
      (product) => String(product.id) === productId,
    );

    return selectedProduct?.currentStock ?? null;
  };
  ...
  const validateForm = () => {
    const errors: SaleOrderFormErrors = {};

    if (!form.partnerId) {
      errors.partnerId = '고객사를 선택해 주세요.';
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setFormErrors({
          submit: data.message || '주문 등록 중 오류가 발생했습니다.',
        });
        return;
      }

      closeCreateModal();
      await fetchSaleOrders(search, 1);
    } catch (error) {
      console.error(error);
      setFormErrors({
        submit:
  ...
  const handleChangeStatus = async (
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.message || '주문 상태 변경 중 오류가 발생했습니다.',
        );
      }
      ...
      const refreshData: SaleOrderListResponse = await refreshResponse.json();

      if (!refreshResponse.ok) {
        throw new Error(
          (refreshData as { message?: string }).message ||
            '주문 상태 변경 후 목록을 불러오는 중 오류가 발생했습니다.',
        );
      }

      setSaleOrders(Array.isArray(refreshData.items) ? refreshData.items : []);
              ...
              >
                ...
                <div className="mt-4 grid grid-cols-[minmax(0,2.2fr)_minmax(90px,0.8fr)_minmax(90px,0.8fr)_minmax(140px,1fr)_auto] items-center gap-4 rounded-xl bg-[var(--color-background)] p-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-[15px] font-semibold text-[var(--color-text-primary)]">
                      {order.saleOrderProducts.length}건
                    </p>
                  </div>
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-[15px] font-semibold text-[var(--color-text-primary)]">
                      {order.saleOrderProducts.reduce(
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    {getFilteredCustomers(form.partnerKeyword).map(
                      (customer) => (
          ...
          <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-2xl border border-[var(--color-border)] bg-[var(--color-white)] p-4 shadow-xl md:p-6">
            ...
            <div className="hidden overflow-hidden rounded-xl border border-[var(--color-border)] md:block">
              <table className="w-full table-fixed border-collapse">
                ...
                <tbody>
                  {selectedOrder.saleOrderProducts.map((item) => (
                    <tr
            ...
            <div className="mt-6">
              ...
              {selectedOrder.saleOrderStatuses.length === 0 ? (
                ...
                <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-white)]">
                  {selectedOrder.saleOrderStatuses.map((history, index) => (
                    <div
                      key={history.id}
                      className={`flex items-center justify-between gap-4 px-4 py-4 ${
                        index !== selectedOrder.saleOrderStatuses.length - 1
                ...
                <div className="flex flex-col-reverse gap-3 md:flex-row md:items-center">
                  ...
                  {selectedOrder.status === 'RECEIVED' && (
                    <button
                      type="button"
                      onClick={() =>
                        handleChangeStatus(selectedOrder.id, 'PROCESSING')
                      }
